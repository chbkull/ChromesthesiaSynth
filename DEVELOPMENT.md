...
Update after a few hours: after doing some major digging in the source code of the ofxSTK library, I noticed a key difference between it and the STK library itself: for some reason my system was not being recognized as a windows system (__WINDOWS_DS__). By some miracle I happened to stumble across (this issue post)[https://github.com/TonicAudio/Tonic/issues/265] about a different sound library, where they added two things to pre-processor definitions (__\_\_WINDOWS_DS\_\___ and __WIN32__). Upon adding that, the framework now successfully compiles, and I am able to press a key and play a sound via ofxStk. Sweet victory.
